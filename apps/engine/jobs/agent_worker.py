...
async def _run_job(job_id: str, cancel_flag: asyncio.Event) -> None:
    ...
    def log(msg: str) -> None:
        logs.append(f"[{datetime.now(UTC).strftime('%H:%M:%S')}] {msg}")
    ...
    try:
                                ...
                                *_memory_msgs(memory),
                                {
                                    "role": "user",
                                    "content": (
                                        f"Describe '{p.get('name', '')}' "
                                        f"({p.get('lore_type', 'term')}) in 2-3 sentences."
                                    ),
                                },
                            ]
                        )
                        ...
                        g = await client.chat_messages(
                            [
                                ...
                                *_memory_msgs(memory),
                                {
                                    "role": "user",
                                    "content": f"Write chapter '{p.get('title', 'Chapter')}'. 400-600 words.",
                                },
                            ]
                        )
                        p = {**p, "content": g.strip()}
                        ...
                        g = await client.chat_messages(
                            [
                                ...
                                *_memory_msgs(memory),
                                {
                                    "role": "user",
                                    "content": f"Describe event '{p.get('title', 'Event')}' in 1-2 sentences.",
                                },
                            ]
                        )
                        p = {**p, "description": g.strip()}
        ...
        for kind, label in [
            ...
            if items:
                names = ", ".join(s["result"].get("name") or s["result"].get("title") or "?" for s in items)
