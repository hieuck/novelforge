...
async def _get_settings() -> ProviderSettings:
    ...
    try:
        row = (
            db.query(Settings)
            .filter(Settings.id == "app-settings")
            .first()
        )
        if not row:
...
def _build_system_prompt(action: str, character_context: str, lore_context: str, timeline_context: str, style_context: str) -> str:
...
async def _inject_summaries(system_prompt: str, project_id: str) -> str:
    ...
    for summary in summaries:
        summary_blocks.append(f"[{summary.kind}] {summary.text}")
    return system_prompt + "\n\nProject summaries:\n" + "\n\n".join(summary_blocks)
...
class AIEngine:
    ...
    async def run(self, action: str, text: str | None, instruction: str | None, chapter_id: str | None) -> str:
        ...
        logger.info("AI request action=%s provider=%s model=%s", action, settings.provider, settings.model)
        return await client.chat(system=system_prompt, user=user_prompt)

    def context(self) -> ProjectContext:
        return self.context


def _build_user_prompt(action: str, text: str, instruction: str, chapter_context: str) -> str:
    prefix_map = {
