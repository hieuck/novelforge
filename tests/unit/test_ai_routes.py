...
def _pid():
    return client.post("/api/projects/", json={"title": "AI"}).json()["id"]


def _cid(pid):
    return client.post(
        "/api/chapters/", json={"project_id": pid, "title": "C", "content": "The hero stood at the gate."}
    ).json()["id"]


def _mock(text="AI result"):
    return patch("routes.ai.build_client", return_value=MagicMock(chat=AsyncMock(return_value=text)))


def test_ai_run_basic():
    with _mock("result"):
        r = client.post("/api/ai/run", json={"action": "continue", "text": "start"})
...
def test_ai_run_with_project():
    ...
    cid = _cid(pid)
    with _mock("cont"):
        r = client.post("/api/ai/run", json={"action": "continue", "project_id": pid, "chapter_id": cid, "text": "x"})
...
def test_ai_run_all_actions():
    for a in [
        ...
        with _mock("out"):
            r = client.post("/api/ai/run", json={"action": a, "text": "t"})
        assert r.status_code == 200, f"{a} failed"


def test_ai_run_with_instruction():
    with _mock("instructed"):
        r = client.post("/api/ai/run", json={"action": "rewrite", "text": "orig", "instruction": "dramatic"})
    assert r.status_code == 200


def test_ai_run_empty_text():
    with _mock("ok"):
        r = client.post("/api/ai/run", json={"action": "premise"})
    assert r.status_code == 200
...
def test_ai_context_includes_character():
    pid = _pid()
    client.post("/api/characters/", json={"project_id": pid, "name": "Zara", "role": "villain"})
    cap = {}

    async def cc(*, system, user):
        cap["s"] = system
        return "ok"
...
def test_ai_context_includes_lore():
    pid = _pid()
    client.post("/api/lore/", json={"project_id": pid, "name": "Shadow Realm", "lore_type": "location"})
    cap = {}
...
def test_ai_context_includes_style_guide():
    pid = _pid()
    client.patch(f"/api/projects/{pid}", json={"style_guide": "Write in second person."})
    cap = {}
