...
def _pid():
    return client.post("/api/projects/", json={"title": "J"}).json()["id"]


def test_create_minimal():
    pid = _pid()
    r = client.post("/api/jobs", json={"project_id": pid, "kind": "outline"})
...
def test_create_with_params():
    pid = _pid()
    r = client.post("/api/jobs", json={"project_id": pid, "kind": "summarize", "params": {"depth": 3}})
    assert r.json()["params"]["depth"] == 3


def test_get_by_id():
    pid = _pid()
    job = client.post("/api/jobs", json={"project_id": pid, "kind": "continuity"}).json()
...
def test_cancel_queued():
    pid = _pid()
    job = client.post("/api/jobs", json={"project_id": pid, "kind": "plot_holes"}).json()
...
def test_cancel_idempotent():
    pid = _pid()
    job = client.post("/api/jobs", json={"project_id": pid, "kind": "x"}).json()
...
def test_websocket_stream():
    pid = _pid()
    job = client.post("/api/jobs", json={"project_id": pid, "kind": "test"}).json()
